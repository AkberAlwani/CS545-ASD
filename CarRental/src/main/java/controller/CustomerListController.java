...
public class CustomerListController extends Application implements Initializable {
	...
	private void populateCustomer() {
		try {
			...
			while (result.next()) {
				...
				cus.setAddress(new org.rentframework.core.Address());
	...
	public void start(Stage primaryStage) throws Exception {
		...
		this.primaryStage.setTitle("Customer List Form");
		try {
			FXMLLoader loader = new FXMLLoader(MainForm.class.getResource("CustomerListForm.fxml"));
	...
	protected void AddCustomer(ActionEvent event) throws Exception {
		CustomerController addcustomer = new CustomerController(0, 0);
	...
	protected void editCustomer(ActionEvent event) throws Exception {
		...
		CustomerController addCustomerController = new CustomerController(customerId, addressId);
		Stage stage = new Stage();
		((Node) (event.getSource())).getScene().getWindow().hide();
